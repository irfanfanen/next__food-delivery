    ...
    <section className="px-8 py-11 container mx-auto lg:px-28">
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {categoriesData.map((item, index) => (
            <div
              key={index}
              className={`${
                item.bgColor ? `bg-[${item.bgColor}]` : "bg-[#F9FFF6]"
              } px-20 py-8 text-center rounded-lg hover:cursor-pointer`}
            >
        ...
        <div className="flex justify-end gap-4">
          <button className="flex items-center gap-2 bg-primary px-3 py-2.5 text-base font-medium rounded-3xl text-white">
